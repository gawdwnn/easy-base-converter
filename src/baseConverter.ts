import { lettersToNumbersMap } from './constants/lettersToNumbersMap';
...
export class BaseConverter {
  ...
  private validateNumberInBaseN(numberToConvert: string, baseFrom: number): void {
    ...
    const splitNumberByDelimeter = numberToConvert.split(this.delimeter);
    if (splitNumberByDelimeter.length < 1 || splitNumberByDelimeter.length > 2) {
      throw new Error('Number contains more than one delimeter');
    }
  ...
  convertFromBaseNToDecimal(numberToConvert: string, baseFrom: number): number {
    // 1101 (binary) => 2^3*1 + 2^2*1 + 2^1*0 + 2^0*1 + 2^(-1)*1 + 2^(-2)*1
    // = 8 + 4 + 0 + 1 + 1/2 + 1/4 = 13.75
    this.validateNumberInBaseN(numberToConvert, baseFrom);
    const digitsArray = Array.from(numberToConvert).filter((digit) => digit !== this.delimeter);
    let position = numberToConvert.split(this.delimeter)[0].length - 1;
    let result = 0;
  ...
  convertFromDecimalToBaseN(numberToConvert: number, baseTo: number, precision = 2): string {
    ...
    const decimalPart = numberToConvert % 1;
   ...
   * @param _namedParameters An object with the options or the convert method to work
